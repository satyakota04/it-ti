...
# Terminal 4: run the slim CallGraphIT suite (agent on test JVM)
./run-it-ti.sh
...
## Slim test suite (CallGraphIT)

A = order, B = inventory, C = shipping. Two tests only:

1. **A→B→C** — `POST /orders` + `GET /orders/{id}`
2. **A↔B** — `GET /stock-order/{sku}` (inventory → order)
